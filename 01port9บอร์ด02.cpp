...
// ============================================================
//  Board Configuration - BOARD 01
// ============================================================
#define BOARD_ID "esp32_node_01"
#define BOARD_NAME "ESP32 Node 01"
#define SENSOR_ID "us_01"
#define SOIL_SENSOR_ID "soil_01"
...
bool connectWiFi() {
  ...
  // พยายามเชื่อมต่อกับ AP ที่สัญญาณแรงที่สุดในรายการ
...
// ============================================================
//  Send Heartbeat (BOARD 01)
//  🔥 สำคัญ: ใช้ lastSeen/.sv (Server Timestamp) เท่านั้น
//  ห้ามใช้เวลาในเครื่อง (millis()) เพราะจะไม่ตรงกับ Firebase
// ============================================================
void sendHeartbeat() {
  if (!Firebase.ready()) {
  ...
  if (isFirstConnection) {
    ...
    json.set("lastSeen/.sv", "timestamp");  // 🔥 Server Timestamp
    ...
    // 🔥 สำคัญ: ใช้ .sv (Server Timestamp) เท่านั้น
    // ไม่มีการตั้งค่า status = "online" โดยตรง
    json.set("lastSeen/.sv", "timestamp");   // 🔥 Server Timestamp
    json.set("wifi_rssi", WiFi.RSSI());
    json.set("install_height", installHeight);
    json.set("bank_height", bankHeight);
...
// ============================================================
void loop() {
  // --- Check WiFi every 5 seconds (ใช้ WiFiMulti) ---
  if (millis() - lastWiFiCheck > 5000) {
    lastWiFiCheck = millis();
    if (WiFi.status() != WL_CONNECTED) {
      Serial.println("⚠️ WiFi disconnected! Attempting to reconnect...");
      connectWiFi(); // WiFiMulti จะจัดการเลือก AP ที่ดีที่สุดให้อัตโนมัติ
    }
  }
  ...
  //  🔥 สำคัญ: ใช้ lastSeen/.sv (Server Timestamp)
  // ============================================================
